...
export class LaunchpadService {
  ...
  public async getLaunchpadState(): Promise<LaunchpadState | null> {
    try {
      ...
      if (data.length < 91) {
        console.warn('Launchpad state data too short, returning mock data');
        return {
          authority: SystemProgram.programId,
          treasuryAuthority: SystemProgram.programId,
      ...
      return {
        ...
        treasuryAuthority: safePublicKey(data, 40, 72),
  ...
  public async getLaunchState(creator: PublicKey): Promise<LaunchState | null> {
    try {
      ...
      // Validate data length
      if (data.length < 264) {
        console.warn('Launch state data too short, returning mock data');
        return {
          creator: creator,
          tokenMint: SystemProgram.programId,
          tokenName: 'Mock Token',
          tokenSymbol: 'MOCK',
          tokenUri: 'https://example.com/mock.json',
          softCap: 1000000000, // 1 SOL
          hardCap: 10000000000, // 10 SOL
          tokenPrice: 100000000, // 0.1 SOL
          tokensForSale: 1000000000, // 1 billion tokens
          minContribution: 100000000, // 0.1 SOL
          maxContribution: 1000000000, // 1 SOL
          launchStart: Date.now() / 1000,
          launchEnd: Date.now() / 1000 + 86400 * 7, // 7 days
          totalRaised: 0,
          totalContributors: 0,
          tokensSold: 0,
      ...
      return {
        creator: safePublicKey(data, 8, 40),
        tokenMint: safePublicKey(data, 40, 72),
  ...
  public async getContributorState(
    ...
    try {
      ...
      // Validate data length
      if (data.length < 89) {
        console.warn('Contributor state data too short, returning mock data');
        return {
          contributor: contributor,
          launch: launch, // Use the passed launch parameter as fallback
          totalContributed: 0,
          tokensOwed: 0,
          claimed: false,
        };
      ...
      return {
        contributor: safePublicKey(data, 8, 40),
        launch: safePublicKey(data, 40, 72),
